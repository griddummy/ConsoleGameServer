...
namespace ConsoleGameServer
{
    public class TcpServer
    {
        ...
        public void DisconnectClient(Socket client)
        {
            ...
            {
                Console.WriteLine("TCPSERVER::DisconnectClient() 실패");
            }
            Console.WriteLine("TCPSERVER::DisconnectClient() 남은 클라 - " + clientSockes.Count);
        }
        ...
        private void HandleAsyncAccept(IAsyncResult asyncResult)
        {
            ...
            Console.WriteLine("TcpServer::Accept " + clientSock.RemoteEndPoint.ToString());
            if (OnAccepted != null)
            {
            ...
            try
            {
                clientSock.BeginReceive(asyncData.msg, 0, AsyncData.msgMaxLength, SocketFlags.None, asyncReceiveCallback, ob);
            }
            catch
            {
        ...
        private void HandleAsyncReceive(IAsyncResult asyncResult)
        {
            ...
            {
                Console.WriteLine("TcpServer::HandleAsyncReceive() : EndReceive - 예외");
                DisconnectClient(clientSock);
                return;
            }
            if (clientSock.Connected)
            {
                if (OnReceived != null)
                {
                    ...
                    {
                        Console.WriteLine("TcpServer::HandleAsyncReceive() : OnReceived - 예외 - 자료없음");
                        DisconnectClient(clientSock);
                    }
                }
            }
            ...
            {
                Console.WriteLine("TcpServer::HandleAsyncReceive() : OnReceived - 예외 - 커넥 없음");
                DisconnectClient(clientSock);
            }
            ...
            {
                Console.WriteLine("TcpServer::HandleAsyncReceive() : BeginReceive - 예외");
                DisconnectClient(clientSock);
            }
        }
        ...
        public void SendAll(byte[] data, int size)
        {
            foreach (Socket client in clientSockes)
            {
                ...
                {
                    Console.WriteLine("TcpServer::SendAll() : Send - 예외");
                }
            }
        }
        ...
        public int Send(Socket _client, byte[] data, int size)
        {
            foreach (Socket client in clientSockes)
            {
                if (client == _client)
                {
                    ...
                    {
                        Console.WriteLine("TcpServer::Send() : Send - 예외");
                    }
                    break;
                }
            }
